    ...
    <section className="py-16 md:py-24 bg-base-200 text-base-content">
      <div className="container mx-auto px-4">
                ...
                <div key={index} className="flex items-center">
                  ...
                  {info.isLink ? (
                    <a
                      ...
                      className="text-primary text-lg hover:underline ml-2 break-all"
                    >
                      {info.value}
                    </a>
                  ) : (
                    <span className="text-lg text-base-content/80 ml-2">
                      {info.value}
                    </span>
                  )}
                </div>
                ...
                {skills.categories.map((category, index) => (
                  <div 
                    ...
                    className={`card bg-base-100 shadow-xl p-8 transform transition-transform duration-300 hover:-translate-y-2 animate__animated
                        ${index === 0 ? 'animate__fadeInLeft' : index === 1 ? 'animate__fadeInUp' : 'animate__fadeInRight'}
                      `}
                  >
                    <h4 className={`card-title text-base-content mb-6 text-2xl font-semibold border-b pb-3 border-base-content/20 ${category.colorClass}`}>
                      <SquareCode className={`h-8 w-8 mr-3 ${category.colorClass}`}/>
                      {category.name}
                    </h4>
          ...
          <div className="my-10">
              ...
              <ul className="timeline timeline-snap-icon max-md:timeline-compact timeline-vertical lg:pr-28">
                {journey.timelineEvents.map((event, index) => {
                  return (
                    <li key={event.id} className={`animate__animated animate__fadeInUp animate__delay-${index * 0.2}s`}>
                      {index > 0 && <hr />}
                      <div className="timeline-middle">
                        <Component className={`h-6 w-6 ${event.colorClass}`} />
                      </div>
                      <div className={`timeline-end timeline-box bg-base-100 shadow-xl p-6 mb-10 ${index % 2 === 0 ? '' : 'md:timeline-start md:text-end'}`}>
                        <div className={`font-bold text-xl mb-2 ${event.colorClass}`}>{event.title}</div>
                        <div className="text-sm text-base-content/70 mb-3">{event.date}</div>
        ...
        <div className="text-center mt-16 animate__animated animate__fadeInUp animate__delay-1s">
          <h3 className="text-3xl lg:text-4xl font-bold text-base-content mb-8">{finalCta.title}</h3>
            ...
            {finalCta.buttons.map((button, index) => {
              return (
                <a
                  ...
                  onClick={(e) => { e.preventDefault(); document.getElementById(button.link.substring(1)).scrollIntoView({ behavior: 'smooth' }); }}
                >
